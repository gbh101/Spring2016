...
class xRumble
{
    ...
    public float duration;  // Fade-out Timer (in seconds)
...
public class x360_Gamepad : MonoBehaviour
{
    ...
    private xButton A, B, X, Y;             // Face Buttons
    private xButton DPad_Up, DPad_Down, DPad_Left, DPad_Right; // D-Pad Buttons

    private xButton Guide;                  // Xbox Button
    private xButton Back, Start;

    private xButton L3, R3;                 // Thumbstick Buttons
    private xButton LB, RB;                 // Bumpers
    ...
    {
        // Set gamepad index
        gamepadIndex = index - 1;           // Allows use of 1-4 instead of 0-3
    ...
    public void Update()
    {
        ...
        // Check gamepad is connected before storing
    ...
    public void Refresh()
    {
        // This stores the current state for the next update
        prev_state = state;
